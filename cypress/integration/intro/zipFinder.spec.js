describe('ZipFinder', () => {
    ...
    it('finds correct text', () => {
        cy.get('[data-cy=title]').contains('Enter your zip code to see your Federal elections');
        cy.get('[data-cy=form-label]').contains('Enter Home Zip Code');
        cy.get('[data-cy=geo-location-button]').contains('I’m Home, Use my Current Location');
    ...
    it('check invalid numeric zipcode', () => {
        ...
        cy.get('[data-cy=zipcode]').get('input').should('have.value', '11111')
    ...
    it('check invalid non-numeric zipcode', () => {
        ...
        cy.get('[data-cy=submit]').click()
        cy.get('[data-cy=alert').contains('Invalid Zip Code')
        cy.get('[data-cy=zipcode]').get('input').should('have.value', 'aaaaa')
    });
});
