...
Cypress.Commands.add('testFAQ', ($el, index, article) => {
    cy.wrap($el).should('have.attr', 'href').and('include', `?article=${article.id}`);
    cy.wrap($el).find('[data-cy=faq-title]').contains(article.title)
});

Cypress.Commands.add('testFAQArticle', ($el, index, article) => {
    cy.wrap($el).find('[data-cy=faq-title]').click()
    cy.url().should('include', `?article=${article.id}`)
    cy.get('[data-cy=page-title]').contains(article ? article.title : 'FAQ Article');

    cy.get('[data-cy=article-title]').contains(article.title);

    cy.get('[data-cy=was-helpful]').contains('Was this helpful?');
    cy.get('[data-cy=helpful-yes]').contains('Yes');
    cy.get('[data-cy=helpful-no]').contains('No');
